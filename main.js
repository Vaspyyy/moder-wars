...
function updateWarDebugOverlay(countryStats, shouldCountLand, combatantIds) {
    ...
    const lines = [];
    lines.push(`MW DEBUG  tick=${simFrameCount}  speed=${simSpeed.toFixed(2)}  units=${units.length}`);
    if (_debugWarSummary) {
        const b = _debugWarSummary.bounds;
        const bTxt = b ? `${b.minLat.toFixed(1)},${b.minLng.toFixed(1)} -> ${b.maxLat.toFixed(1)},${b.maxLng.toFixed(1)}` : 'n/a';
        lines.push(`warCells=${_debugWarSummary.warCells} orphanWarCells=${_debugWarSummary.orphanWarCells} bounds=${bTxt}`);
    } else {
...
function updatePersistentInfluence(p1Count, p2Count, countryToSideMap) {
    ...
    const citySource = (activeTheaterCities && activeTheaterCities.length) ? activeTheaterCities : [];
    ...
    const sideSupportIdSets = sides.map(side => {
        const s = new Set();
        side.forEach(c => { if (c.role === 'SUPPORT') s.add(c.id); });
        return s;
    });
        ...
        for (let y = startLat; y <= endLat; y++) {
            for (let x = startLng; x <= endLng; x++) {
                ...
                if (dSq < rVar * rVar) {
                    ...
                    const isOwnerAlly = sideAllyIdSets[u.sideIndex] && sideAllyIdSets[u.sideIndex].has(ownerId);
                    ...
                    if (!isOwnerAlly && ownerId > 0 && role === 'OFFENSE') {
                        ...
                        if (ownerSideIdx !== undefined && (ownerSideIdx % 2 !== u.sideIndex % 2)) {
                            if (sideSupportIdSets[ownerSideIdx] && sideSupportIdSets[ownerSideIdx].has(ownerId)) {
                                const occ = occupationMap[idx];
...
function performSimulationTick() {
    ...
    const p1T = _cachedP1T, p2T = _cachedP2T;
    ...
    if (shouldCountLand) {
        ...
        sides.flat().forEach(c => {
            const stats = countryStats.get(c.id);
            if (stats) {
                stats.controlled = c.lastControlledCount !== undefined ? c.lastControlledCount : (c.initialCells || 0);
                stats.owned = c.lastOwnedCount !== undefined ? c.lastOwnedCount : (c.initialCells || 0);
            }
        });
    }
    ...
    if (cities && cities.length) {
        for (let ci = 0; ci < cities.length; ci++) {
            ...
            if (oid !== null) {
                ...
                if (!arr) { arr = []; _citiesBySovereign.set(oid, arr); }
                arr.push(c);
            }
        }
    }
        ...
        if (target) {
            ...
            if (dist > 0.05 && !isEngaged) {
                ...
                const movedEnoughForNeutralCheck = (
                    ...
                    (u.lat - lastNeutralLat) ** 2 + (u.lng - lastNeutralLng) ** 2 >= NEUTRAL_CHECK_DIST_SQ
                );
                ...
                if ((isProtectedSupport(lookIdx) || isInsideNeutralProtected) && movedEnoughForNeutralCheck) {
                ...
                } else if ((isProtectedSupport(lookIdx) || isInsideNeutralProtected) && !movedEnoughForNeutralCheck) {
        ...
        for (let i = side.length - 1; i >= 0; i--) {
            ...
            if (!shouldCountLand && stats.units === 0 && controlPct < 35) {
                ...
                for (let idx = 0; idx < worldControlMap.length; idx++) {
                    if (landMask[idx] === 2 && worldControlMap[idx] === country.id) exactOwned++;
                }
            ...
            const nearlyErasedNoUnits = (stats.units === 0) && (liveOwnedWarTiles <= Math.max(1, Math.floor(initial * 0.003)));
            const heldCities = countryToCityCount.get(country.id) || 0;
            const noForcesNoCities = (stats.units === 0) && (heldCities === 0);
            if (!isProtected && (hasNoOwnedWarTiles || nearlyErasedNoUnits || noForcesNoCities || (stats.units === 0 && controlPct < 25) || controlPct < 2)) {
...
async function initMultiplayer() {
    ...
    if (typeof WebsimSocket === 'undefined') {
        console.warn('Multiplayer disabled: WebsimSocket is unavailable in local server mode.');
        return;
    }
